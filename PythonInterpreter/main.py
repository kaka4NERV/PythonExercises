import dis,sys,collections,operator
...
class VirtualMachine(object):
    ...
    def push_block(self, b_type, handler=None):
        ...
        self.fram.block_stack.append(Block(b_type, handler, stack_height))
    ...
    def bytes_LOAD_FAST(selfs, name):
    ...
    def binaryOperator(self, op):
        x, y = self.popn(2)
        self.push(self.BINARY_OPERATOR[op](x, y))
    ...
    def byte_BULD_MAP(self, size):
        self.push({})

    def byte_STORE_MAP(self):
        the_map, val, key = self.popn(3)
        the_map[key] = valself.push(the_map)
        self.push(the_map)
    ...
    def byte_FOR_ITER(selfself, jump):
        iterobj = self.top()
        try:
            v = next(iterobj)
            self.push(v)
